...
public:
   ...
   void              Fill2(int x,int y,const uint clr);
   ...
   void              LineAA2(const double x1,const double y1,const double x2,const double y2,const uint clr,const uint style=UINT_MAX);
   ...
   void              TextSize(const string text,uint &width,uint &height);
   //--- services
...
//--- if arc is obviously not within the rays range, don't draw
   ...
   if((fi3<M_PI_2 || fi3>3*M_PI_2) ||  // ray 3 falls in 1 or 4 quadrant
      (fi<M_PI_2 || fi>3*M_PI_2)   ||  // ray 4 falls in 1 or 4 quadrant
       fi4-fi3>=M_PI)                  // arc passes through the right part of the ellipse
     {
      dx=rx;
      dy=0;
      f=(ry_sq<<1)*((dx-1)*dx)+ry_sq+(rx_sq<<1)*(1-ry_sq);
...
//| Text rectangle                                                   |
//+------------------------------------------------------------------+
void CCanvas::TextSize(const string text,uint &width,uint &height)
  {
   if(FontSet())
